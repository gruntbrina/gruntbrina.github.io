...
function enemyConditions(j) {
	...
	if (distBombEnemy >0) {
		//Special Effects Gif
		var gifAttack = paper.image("resources/EnemyExplodes.gif",enemySpawnPosition[j].x,enemySpawnPosition[j].y+j*box.y,box.x,box.y)
		setTimeout(function(){gifAttack.remove()},400)
	...
	if (distCannonEnemy > 0) {
		//Special Effects Gif
		var gifAttack = paper.image("resources/DestroyCannonBallGif.gif",enemySpawnPosition[j].x,enemySpawnPosition[j].y+j*box.y,box.x,box.y)
		setTimeout(function(){gifAttack.remove()},500)
...
//It is placed at the bottom because I want players to be able to interact with it before starting the game (For the reset highscore button)
...
var soundOnOff = paper.text(soundOnOffInfo.x,soundOnOffInfo.y, "Sound: On")
soundOnOff.attr({
	'font-size': 20
});

//To turn the sound on and off
soundOnOff.node.addEventListener('click', function(ev){
	if(soundState == 1) {
		bgMusic.muted = true;
		gameStartSound.muted = true;
		playerStepSound.muted = true;
		pickUpBombSound.muted = true;
		loadCannonSound.muted = true;
		fireCannonSound.muted = true;
		enemyDeathSound.muted = true;
		enemyBombCollisionSound.muted = true;
		enemyCannonCollisionSound.muted = true;
		...
		playerStepSound.muted = false;
		pickUpBombSound.muted = false;
		loadCannonSound.muted = false;
		fireCannonSound.muted = false;
		enemyDeathSound.muted = false;
		enemyBombCollisionSound.muted = false;
		enemyCannonCollisionSound.muted = false;
...
//https://i.pinimg.com/originals/10/92/fa/1092fa7d42c7db063aaf5f1d517b1692.gif
//https://himekosutori.com/wp-content/uploads/2017/10/YellowSwing.gif
